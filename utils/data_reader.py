...
def get_minibatch_with_class(input_seqs, tag_seqs, class_labels, word2idx, tag2idx, class2idx, train_data_indx, index, batch_size, add_start_end=False, multiClass=False, keep_order=False, enc_dec_focus=False, device=None, dropout_rate_of_values=0, idx2tag={}):
    """Prepare minibatch."""
    ...
    if dropout_rate_of_values <= 0:
        ...
        input_idxs = [
            [w if idx2tag[t][0] not in {'B', 'I'} or random.random() < 1 - dropout_rate_of_values else word2idx['<unk>'] for w,t in zip(seq,tag)] + [word2idx['<pad>']] * (max_len - len(seq))
            for seq,tag,_ in data_mb
            ]
